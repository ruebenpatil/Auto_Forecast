...
def load_and_parse_date(df, date_column="Date"):
    parsed = False
    formats_to_try = ["%Y-%m-%d", "%d-%m-%Y", "%m/%d/%Y", "mixed"]
    ...
    if not parsed:
        try:
            ...
            logger.error(
                "Unable to parse the 'Date' column. Please use consistent date format."
            )
            raise e
...
def preprocess_data(df, target_column="Target"):
    if len(df) < MIN_DAILY_DATA_SPAN:
        logger.error(f"Insufficient data. Provide at least (~{MIN_DAILY_DATA_SPAN} days) of data.")
        sys.exit()
    ...
    if not outliers.empty:
        ...
        df.loc[df[target_column] < lower_bound, target_column] = df[
            target_column
        ].median()
        df.loc[df[target_column] > upper_bound, target_column] = df[
            target_column
        ].median()
...
def select_lag_features(df, target_column="Target"):
    acf_vals = acf(df[target_column], nlags=30)[1:]
    pacf_vals = pacf(df[target_column], nlags=30)[1:]
    selected_lags = [
        i + 1
        for i, (a, p) in enumerate(zip(acf_vals, pacf_vals))
        if abs(a) > 0.2 or abs(p) > 0.2
    ]
    ...
    while len(selected_features) > 1:
        ...
        vif_data["VIF"] = [
            variance_inflation_factor(df[selected_features].values, i)
...
def select_best_lags(
    train_features, test_features, train_target, test_target, model, max_lags=5
):
...
def optimize_rolling_window(df, target_column="Target", min_window=3, max_window=36):
    ...
    for window in range(min_window, max_window + 1):
        ...
        rmse = np.sqrt(
            mean_squared_error(test[target_column], rolling_mean[-len(test) :])
        )
        if rmse < best_rmse:
            best_rmse = rmse
            best_window = window
    ...
    df[target_column] = (
        df[target_column].rolling(window=best_window, min_periods=1).mean()
    )
    return df


# === Workflow ===
def full_pipeline(df, target_column="Target"):
    ...
    selected_features = select_best_lags(
...
def compute_daily_forecast(df_daily):
    df, X_train, X_test, y_train, y_test, features = full_pipeline(df_daily)
    daily_result = evaluate_ts_models(y_train, y_test, X_train, X_test)
